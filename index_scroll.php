    ...
    foreach ($items as $r) {
      $html .= '<tr>
        <td>
          ...
          ($r->firstImageUrl ? safe_string($r->firstImageUrl) : '/static/img/no-image.png') . 
...
async function loadMoreData() {
    ...
    try {
        ...
        if (data.success) {
            campingTable.insertAdjacentHTML('beforeend', data.html);
...
boardContainer.addEventListener('scroll', function() {
    ...
    scrollTimeout = setTimeout(() => {
        if (boardContainer.scrollHeight - boardContainer.scrollTop 
            <= boardContainer.clientHeight + 200) {
            loadMoreData();
        }
        scrollTimeout = null;
    }, 100);
